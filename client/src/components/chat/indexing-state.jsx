...
export function IndexingState({
  ...
  const filesProcessed =
    status?.filesProcessed ?? repo.filesProcessed;

  const filesTotal =
    status?.filesTotal ?? repo.filesTotal;

  const chunkCount =
    status?.chunkCount ?? repo.chunkCount;
  ...
  const errorMessage =
    status?.errorMessage ?? repo.errorMessage;
